...
Автоматически проверять сборку Docker-образа сервиса для pull request в `main` и публиковать актуальный образ в GitHub Container Registry после каждого push в `main`.
...
- `pull_request` в `main` — образ только собирается, публикации нет;
- `push` в `main` — образ собирается и публикуется в GHCR.
...
Workflow настроит Docker Buildx, выполнит вход в `ghcr.io` только для push в `main`, соберёт Dockerfile из корня репозитория и задействует cache GitHub Actions для слоёв Docker. При pull request параметр публикации отключён, поэтому workflow проверяет, что Dockerfile и build context остаются рабочими, без изменения registry.

Сбои checkout, аутентификации, Docker build или публикации завершают job с ошибкой и видны в GitHub Actions.

## Проверка

Конфигурация workflow будет проверена actionlint. Синтаксис и параметры Docker workflow будут проверены локально; фактическая публикация происходит только в GitHub Actions после merge/push в `main`.
